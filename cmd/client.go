...
func client(config *clientConfig) {
	...
	// Client
	client, err := core.NewClient(config.Server, auth, tlsConfig, quicConfig, tr,
	...
	if len(config.SOCKS5.Listen) > 0 {
		go func() {
			...
			socks5server, err := socks5.NewServer(client, tr, config.SOCKS5.Listen, authFunc,
	...
	if len(config.HTTP.Listen) > 0 {
		go func() {
			...
			proxy, err := hyHTTP.NewProxyHTTPServer(client, tr,
	...
	if len(config.TUN.Name) != 0 {
		go func() {
			...
			tunServer, err := tun.NewServer(client, tr,
	...
	if len(config.TCPRelay.Listen) > 0 {
		go func() {
			rl, err := relay.NewTCPRelay(client, tr,
	...
	if len(config.UDPRelay.Listen) > 0 {
		go func() {
			rl, err := relay.NewUDPRelay(client, tr,
	...
	if len(config.TCPTProxy.Listen) > 0 {
		go func() {
			rl, err := tproxy.NewTCPTProxy(client, tr,
	...
	if len(config.UDPTProxy.Listen) > 0 {
		go func() {
			rl, err := tproxy.NewUDPTProxy(client, tr,
